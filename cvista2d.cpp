    ...
    ui(new Ui::CVista2D)
{
   ...
   // comunicacion_servidor = new CComunicacionServidor;


    ui->label->setText("Escuchando");
  ...
  //  connect(comunicacion_cliente, SIGNAL(NuevaConexion(CConexion*)), this, SLOT(NuevaConexion(CConexion*)));
...
void CVista2D::processPendingDatagrams()
{
//! [2]
   // if(ui->checkBoxRemoto->isChecked())
    //{
    ...
    while (udpSocket->hasPendingDatagrams()) {
        ...
        QString mensaje = datagram.data();
        ui->label->setText(mensaje);

        QStringList Comandos = mensaje.split(",");  // Dividimos el mensaje en trozos




        QStringList Esferas = Comandos[0].split(" ");
        QStringList Cajas = Comandos[1].split(" ");
        QStringList Personas = Comandos[2].split(" ");
        QStringList Robots = Comandos[3].split(" ");

//ui->label->setText(Comandos[0]);

          BorrarEsferas();
        // Creamos nuevas esferas
         int numE = 1;
        int maximo = Esferas[0].toInt()*3+1;

            if(Esferas.size()>3)//para que no pinte nada si no hay una esfera almenos
            {
            do
            {
           //  ui->label->setText(QString::number(maximo))
            TPosicion pos;
           // pos.x=1;
           // pos.y=2;
            pos.x=Esferas[numE].toFloat();
            pos.y=Esferas[numE+1].toFloat();
            int color=Esferas[numE+2].toInt();
            NuevaEsfera((TColor)color, pos);

             numE=numE+3;//salta  los valores de 3 en tres
            } while(numE<=maximo-3);
            }


            BorrarCajas();
          // Creamos nuevas cajas
           numE = 1;
          maximo = Cajas[0].toInt()*3+1;

              if(Cajas.size()>3)//para que no pinte nada si no hay una caja almenos
              {
              do
              {
             //  ui->label->setText(QString::number(maximo))
             ...
             // pos.x=1;
             // pos.y=2;
              pos.x=Cajas[numE].toFloat();
              pos.y=Cajas[numE+1].toFloat();
              int color=Cajas[numE+2].toInt();
              NuevaCaja((TColor)color, pos);
            ...
            // Creamos nuevas personas
             numE = 1;
            maximo = Personas[0].toInt()*4+1;

                if(Personas.size()>3)//para que no pinte nada si no hay una caja almenos
                {
                do
                {
               //  ui->label->setText(QString::number(maximo))
               ...
               // pos.x=1;
               // pos.y=2;
                pos.x=Personas[numE].toFloat();
                pos.y=Personas[numE+1].toFloat();
                float angle = Personas[numE+2].toFloat();
                int color=Personas[numE+3].toInt();
                NuevaPersona((TColor)color, pos,angle);

                 numE=numE+4;//salta  los valores de 4 en 4
                } while(numE<=maximo-4);
                }

                BorrarRobots();
              // Creamos nuevos robots
               numE = 1;
              maximo = Robots[0].toInt()*4+1;

                  if(Robots.size()>3)//para que no pinte nada si no hay una caja almenos
                  {
                  do
                  {
                 //  ui->label->setText(QString::number(maximo))
                 ...
                 // pos.x=1;
                 // pos.y=2;
                  pos.x=Robots[numE].toFloat();
                  pos.y=Robots[numE+1].toFloat();
                  float angle = Robots[numE+2].toFloat();
                  int color=Robots[numE+3].toInt();
                  NuevoRobot((TColor)color, pos,angle);
