...
void MainWindow::Interpretar(QString mensaje)
{
    ...
    if (Comandos[0]=="P")
    {
       int id = Comandos[1].toInt();

       if (Comandos[2]=="A")
       {
               float velocidad = Comandos[3].toFloat();
               escena->Persona(id)->VelocidadAvance(velocidad);
       }
       else if (Comandos[2]=="G")
       {
               float velocidad = Comandos[3].toFloat();
               escena->Persona(id)->VelocidadGiro(velocidad);
       }
       else if (Comandos[2]=="P")
       {
           escena->Persona(id)->VelocidadAvance(0.0);
           escena->Persona(id)->VelocidadGiro  (0.0);

       }
       else if (Comandos[2]=="C")
       {
               escena->Persona(id)->Coger(NULL);
       }
       else if (Comandos[2]=="S")
       {
               escena->Persona(id)->Soltar();
       }
       else if (Comandos[2]=="a")
       {
               escena->Persona(id)->Almacenar(NULL);
       }
       else if (Comandos[2]=="p")
       {
               escena->Persona(id)->Patear(NULL);
       }
       else if (Comandos[2]=="EN")
       {
               escena->Persona(id)->Empujar(false);
       }
       else if (Comandos[2]=="ES")
       {
               escena->Persona(id)->Empujar(true);
       }
    }

    else if (Comandos[0]=="R")
    {
        int id = Comandos[1].toInt();
              ...
              {
                      float velocidad = Comandos[3].toFloat();
                      escena->Robot(id)->VelocidadAvance(velocidad);
              }
              else if (Comandos[2]=="G")
              {
                      float velocidad = Comandos[3].toFloat();
                      escena->Robot(id)->VelocidadGiro(velocidad);
              }
              else if (Comandos[2]=="P")
              {
                  escena->Robot(id)->VelocidadAvance(0.0);
                  escena->Robot(id)->VelocidadGiro  (0.0);

              }
              else if (Comandos[2]=="C")
              {
                      escena->Robot(id)->Coger(NULL);
              }
              else if (Comandos[2]=="S")
              {
                      escena->Robot(id)->Soltar();
              }
              else if (Comandos[2]=="a")
              {
                      escena->Robot(id)->Almacenar(NULL);
              }
              else if (Comandos[2]=="p")
              {
                      escena->Robot(id)->Patear(NULL);
              }
              else if (Comandos[2]=="EN")
              {
                      escena->Robot(id)->Empujar(false);
              }
              else if (Comandos[2]=="ES")
              {
                      escena->Robot(id)->Empujar(true);
              }
    }
...
{
    broadcastDatagram();
    ui->textEditMensajes->setText(escena->infoescena());
...
void MainWindow::broadcastDatagram(void)
{
    QString string = escena->infoescena();
    ui->statusLabel->setText(tr("Now broadcasting datagram %1").arg(messageNo));
//! [1]
    QByteArray datagram = string.toUtf8();
    udpSocket->writeDatagram(datagram.data(), datagram.size(),
                             QHostAddress::Broadcast, UDP_PORT);
//! [1]
    ++messageNo;
}
